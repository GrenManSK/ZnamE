...
if nenajdene:
    ...
    print("Prosím znovu zapni program")
    sleep(1)
    quit()
os.system('cls')
...
def main():
    ...
    while True:
        if not exit:
            ...
            if logged and vstup == "logout":
                logged = False
                os.remove(loginvstupuser + 'crypted')
                print("Si odhlásený")
                continue
            elif logged and inactivelogout:
                logged = False
                print("Si odhlásený!!!")
                continue
            ...
            if vstup != "":
                ...
                if vstup == 'login' and not logged or tologin and not logged:
                    ...
                    if tologinhelp:
                        ...
                        print('IČO neobsahuje písmená alebo znaky!!!')
                        tologin = True
                        continue
                    history.write(loginvstupuser + '\n')
                    if len(str(loginvstupuser)) == 6:
                        exit = False
                        icofind = code(find(decode()))
                        if icofind:
                            logged = False
                            os.remove(loginvstupuser + 'crypted')
                            print("ZLÉ IČO!!!")
        ...
        elif not logged and vstup == 'quit' or not logged and vstup == 'koniec' or not logged and exit:
            try:
                open('END', 'x')
            except Exception:
                pass
            ...
            sleep(1)
            quit()
        elif logged and vstup == 'quit' or logged and vstup == 'koniec':
            try:
                open('END', 'x')
            except Exception:
                pass
